...
3. **希望将 PaddleOCR-VL 部署为 API 服务**：
    ...
    如果您想将 PaddleOCR-VL 部署为一个网络服务（API），这样其他设备或应用程序无需配置环境，仅通过一个特定的网址就可以来访问和调用它，我们提供三种方式：

    - 使用 Docker Compose 部署（一键启动，推荐使用）：请阅读 [4.1 方法一：使用 Docker Compose 部署](#41-方法一使用-docker-compose-部署推荐使用) 和 [4.4 客户端调用方式](#44-客户端调用方式)，或其他硬件文档中的对应章节。
    - 进行手动部署：请阅读 [1. 环境准备](#1-环境准备)、 [4.2 方法二：手动部署](#42-方法二手动部署) 和 [4.4 客户端调用方式](#44-客户端调用方式)，或其他硬件文档中的对应章节。
    - 使用 HPS 高性能服务化部署（支持并发请求，推荐生产环境使用）：请阅读 [4.3 方法三：使用 HPS 高性能服务化部署](#43-方法三使用-hps-高性能服务化部署支持并发请求) 和 [4.4 客户端调用方式](#44-客户端调用方式)。
...
此步骤主要介绍如何将 PaddleOCR-VL 部署为服务并调用，有以下三种方式，任选一种即可：

- 方法一：使用 Docker Compose 部署（推荐使用）。

- 方法二：手动部署。

- 方法三：使用 HPS 高性能服务化部署（支持并发请求）。

> 请注意，本节所介绍 PaddleOCR-VL 服务与上一节中的 VLM 推理服务有所区别：后者仅负责完整流程中的一个环节（即 VLM 推理），并作为前者的底层服务被调用。方法一和方法二一次仅能处理一个请求，如需支持并发请求，请使用方法三（HPS 高性能服务化部署）。
...
参考本文中 <a href="#45-产线配置调整说明">4.5 产线配置调整说明</a> 小节。
...
如需调整产线相关配置（如模型路径、批处理大小、部署设备等），可参考 4.5 小节。

### 4.3 方法三：使用 HPS 高性能服务化部署（支持并发请求）

HPS（High Performance Server）基于 FastAPI 网关 + Triton 推理服务器 + vLLM 的三层架构，支持并发请求处理，适合对吞吐量有较高要求的生产环境。
...
> HPS 的 API 格式与方法一、方法二完全一致，客户端调用方式请参考 [4.4 客户端调用方式](#44-客户端调用方式)。
>
> 更多高级配置和故障排查信息，请参阅 [HPS 完整文档](https://github.com/PaddlePaddle/PaddleOCR/blob/main/deploy/paddleocr_vl_docker/hps/README.md)。
