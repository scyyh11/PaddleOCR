...
目前 PaddleOCR-VL 有七种推理方式，支持的推理设备不完全相同，请确认您的推理设备是否满足下表要求再进行 PaddleOCR-VL 的推理部署：
...
“PaddlePaddle” 表示版面检测模型与 VLM 均使用飞桨框架推理，PaddleOCR CLI 与 Python API 默认使用这种推理方式；“Transformers” 表示版面检测模型与 VLM 均通过 Transformers 引擎推理；其余推理方式遵循 “版面检测模型推理方式 + VLM 推理方式” 的格式，如“PaddlePaddle/Transformers + vLLM”是指版面检测模型使用 PaddlePaddle 或 Transformers 推理，VLM 使用 vLLM 推理。
</details>
...
- 使用 PaddlePaddle 推理时：请安装 3.2.1 及以上版本的 PaddlePaddle。以 CUDA 12.6 为例：

```shell
# 对于其他 CUDA 版本以及 CPU 版本，请参考 https://www.paddlepaddle.org.cn/install/quick?docurl=/documentation/docs/zh/develop/install/pip/linux-pip.html
...
# 华为昇腾 NPU 
# 华为昇腾 NPU 请参考第 3 章节使用 PaddlePaddle/Transformers + vLLM 的方式进行推理
...
只使用 PaddlePaddle 或 Transformers 通常无法得到最优的推理性能。此步骤主要介绍如何使用 vLLM、SGLang 和 FastDeploy 推理加速框架来提升 PaddleOCR-VL 的推理性能。
