...
3. **希望将 PaddleOCR-VL 部署为 API 服务**：
    ...
    - 使用 Docker Compose 部署（一键启动，一次处理一个请求，推荐使用）：请阅读 [4.1 方法一：使用 Docker Compose 部署](#41-方法一使用-docker-compose-部署推荐使用) 和 [4.4 客户端调用方式](#44-客户端调用方式)，或其他硬件文档中的对应章节。
    - 手动部署（一次处理一个请求）：请阅读 [1. 环境准备](#1-环境准备)、 [4.2 方法二：手动部署](#42-方法二手动部署) 和 [4.4 客户端调用方式](#44-客户端调用方式)，或其他硬件文档中的对应章节。
    - 高性能服务化部署（基于 Docker Compose，支持并发请求，推荐生产环境使用）：请阅读 [4.3 方法三：高性能服务化部署](#43-方法三高性能服务化部署支持并发请求) 和 [4.4 客户端调用方式](#44-客户端调用方式)。
...
> 请注意，本节所介绍 PaddleOCR-VL 服务与上一节中的 VLM 推理服务有所区别：后者仅负责完整流程中的一个环节（即 VLM 推理），并作为前者的底层服务被调用。方法一和方法二一次仅能处理一个请求，如需支持并发请求，请使用方法三（高性能服务化部署）。
...
### 4.3 方法三：高性能服务化部署（支持并发请求）

高性能服务化部署基于 FastAPI 网关 + Triton 推理服务器 + vLLM 的三层架构，支持并发请求处理，适合对吞吐量有较高要求的生产环境。

> 高性能服务化部署的 API 格式与方法一、方法二完全一致，客户端调用方式请参考 [4.4 客户端调用方式](#44-客户端调用方式)。

详细的环境要求、快速开始、配置说明、性能调优和故障排查等内容，请参阅 [高性能服务化部署完整文档](https://github.com/PaddlePaddle/PaddleOCR/blob/main/deploy/paddleocr_vl_docker/hps/README.md)。
