...
| 引擎类别 | `engine` 取值 | 说明 |
| - | - | - |
| 飞桨框架 | `paddle`、`paddle_static`、`paddle_dynamic` | 基于飞桨框架运行。`paddle` 是统一入口，会根据模型类型和模型目录中的文件选择 `paddle_static` 或 `paddle_dynamic`。 |
| Transformers | `transformers` | 基于 Hugging Face Transformers 运行。 |

- `paddle`：飞桨框架统一入口。
- `paddle_static`：飞桨静态图推理，适合对推理性能有一定要求或者需要进行精细化推理性能调优的场景。
- `paddle_dynamic`：飞桨动态图推理。
- `transformers`：Hugging Face Transformers 推理。
